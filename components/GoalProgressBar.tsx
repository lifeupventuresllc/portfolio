...
export function GoalProgressCompact({
  ...
  if (goal === 'maintain') {
    const inner = (
      <>
        ...
        {/* Log food pill CUT from Home (2026-09-24 audit, Asa's ask) — food
            logging now lives on My Day (/plan/today already has its own
            "+ Log food" entry). CalorieLine above stays — it's the KEEP-
            listed calorie counter, not a logging button. */}
      </>
    )
  ...
  const inner = (
    <>
      ...
      {/* Item 3 of the beta list (2026-09-21, one-tap goal): the weight
      ...
      {/* Log food pill CUT from Home (2026-09-24 audit, Asa's ask) — food
          logging now lives on My Day (/plan/today already has its own
          "+ Log food" entry). budgetLabel above stays untouched — Asa's
          standing rule, it's the KEEP-listed calorie counter, not a
          logging button (see its own comment further up this file). */}
    </>
  )
