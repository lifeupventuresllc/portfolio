...
export async function GET(request: NextRequest) {
  ...
  // --- Email 2: Leads created 2+ days ago, not yet sent ---
  const twoDaysAgo = new Date(now.getTime() - 2 * 24 * 60 * 60 * 1000).toISOString()
  const { data: email2Leads } = await supabase
    .from('funnel_leads')
    .select('id, email, name, notes')
    .eq('service', 'fitness')
    .lte('created_at', twoDaysAgo)
    .not('notes', 'like', '%Email 2 sent%')
    .limit(50)
  ...
  for (const lead of email2Leads || []) {
    try {
      ...
      // Update notes to track that Email 2 was sent
      const existingNotes = lead.notes || ''
      const newNote = `${existingNotes ? existingNotes + '\n' : ''}Email 2 sent: ${now.toISOString()}`

      await supabase
        .from('funnel_leads')
        .update({ notes: newNote, updated_at: now.toISOString() })
        .eq('id', lead.id)

      email2Sent++
    } catch (err) {
      console.error(`Failed to send Email 2 to ${lead.email}:`, err)
    }
  }

  // --- Email 3: Leads created 4+ days ago, not yet sent ---
  const fourDaysAgo = new Date(now.getTime() - 4 * 24 * 60 * 60 * 1000).toISOString()
  const { data: email3Leads } = await supabase
    .from('funnel_leads')
    .select('id, email, name, notes')
    .eq('service', 'fitness')
    .lte('created_at', fourDaysAgo)
    .not('notes', 'like', '%Email 3 sent%')
    .limit(50)
  ...
  for (const lead of email3Leads || []) {
    try {
      ...
      // Update notes to track that Email 3 was sent
      const existingNotes = lead.notes || ''
      const newNote = `${existingNotes ? existingNotes + '\n' : ''}Email 3 sent: ${now.toISOString()}`
      ...
      console.error(`Failed to send Email 3 to ${lead.email}:`, err)
    }
  }
