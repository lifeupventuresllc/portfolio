...
export default function ConversationalIntake() {
  ...
  const Hint = ({ children }: { children: React.ReactNode }) => <p className="text-ivory/50 text-sm mb-7">{children}</p>
  const opt = (active: boolean) => `w-full text-left px-5 py-4 rounded-2xl border text-white font-semibold transition-all duration-200 ${active ? 'bg-gold/15 border-gold scale-[1.01]' : 'bg-charcoal border-smoke hover:border-gold/50 hover:bg-charcoal/70'}`
    ...
    return (
      <div className="min-h-screen bg-obsidian flex items-center justify-center px-6">
        <div className="text-center q-in-fwd">
          ...
          <p className="text-ivory/50 text-sm">Crunching your numbers, matching your workout, and setting up your meals for {firstName}.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-obsidian px-4 py-8 flex flex-col">
      ...
      <div className="max-w-lg w-full mx-auto mb-8">
        <div className="flex items-center gap-3">
          {step > 0 ? <button onClick={back} className="text-ivory/40 hover:text-gold text-sm">←</button> : <span className="w-3" />}
          ...
          <span className="text-ivory/40 text-xs tabular-nums">{step + 1}/{total}</span>
        </div>
      </div>
        ...
        <Screen>
            ...
            <div className="space-y-3">
              {[{ v: 'lose', l: '🔥 Lose fat', d: 'Lean out, keep your curves' }, { v: 'gain', l: '💪🏽 Build & tone', d: 'Add shape and strength' }, { v: 'maintain', l: '⚖️ Maintain', d: 'Hold steady, feel great' }].map((o) => (
                <button key={o.v} onClick={() => pick('goal', o.v)} className={opt(f.goal === o.v)}>
                  <span className="block">{o.l}</span><span className="block text-ivory/40 text-xs font-normal mt-0.5">{o.d}</span>
                </button>
              ))}
            </div>
          </>)}
          ...
          {s === 'body' && (<>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div><label className="text-ivory/40 text-xs uppercase tracking-wider mb-1 block">Age</label><input type="number" value={f.age} onChange={(e) => set('age', e.target.value)} className={input} /></div>
              <div><label className="text-ivory/40 text-xs uppercase tracking-wider mb-1 block">Weight (lbs)</label><input type="number" value={f.weight_lbs} onChange={(e) => set('weight_lbs', e.target.value)} className={input} /></div>
            </div>
            <label className="text-ivory/40 text-xs uppercase tracking-wider mb-1 block">Height</label>
            <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <label className="text-ivory/40 text-xs uppercase tracking-wider mb-1 block">Sex (for your metabolism math)</label>
            <div className="grid grid-cols-2 gap-3 mb-6">
              {[{ v: 'female', l: 'Female' }, { v: 'male', l: 'Male' }].map((o) => <button key={o.v} onClick={() => set('sex', o.v)} className={`py-3 rounded-xl text-sm font-semibold ${f.sex === o.v ? 'bg-gold/15 text-gold border border-gold/40' : 'bg-charcoal border border-smoke text-ivory/60'}`}>{o.l}</button>)}
            </div>
          ...
          {s === 'days' && (<>
            ...
            <div className="grid grid-cols-3 gap-3">
              {['3', '4', '5', '6'].map((d) => (
                <button key={d} onClick={() => pick('days_per_week', d)} className={`py-6 rounded-2xl border text-2xl font-bold ${f.days_per_week === d ? 'bg-gold/15 text-gold border-gold' : 'bg-charcoal border-smoke text-white hover:border-gold/50'}`}>{d}</button>
              ))}
            </div>
          </>)}
          ...
          {s === 'injuries' && (<>
            ...
            <div className="flex flex-wrap gap-2 mb-7">
              {INJURIES.map((i) => (
                <button key={i.v} onClick={() => toggleInjury(i.v)} className={`px-4 py-2.5 rounded-xl text-sm font-semibold ${injuries.includes(i.v) ? 'bg-gold/15 text-gold border border-gold/40' : 'bg-charcoal border border-smoke text-ivory/60'}`}>{i.l}</button>
              ))}
            </div>
          ...
          {s === 'food' && (<>
            ...
            <label className="text-ivory/40 text-xs uppercase tracking-wider mb-1 block">Foods you love</label>
            ...
            <label className="text-ivory/40 text-xs uppercase tracking-wider mb-1 block">Dislikes / allergies</label>
            ...
            <label className="text-ivory/40 text-xs uppercase tracking-wider mb-1 block">Weekly food budget ($)</label>
