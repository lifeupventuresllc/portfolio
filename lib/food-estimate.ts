...
export async function detectEatenFood(message: string): Promise<FoodEstimate[]> {
  ...
  try {
    ...
    const msg = await client.messages.create({
      ...
      max_tokens: 1024,
      system:
        "You are reading a casual message someone sent their fitness coach about their day — often a voice memo transcript where she thinks out loud, restarts a thought, or repeats herself before finishing the sentence. Determine whether she is describing something she ALREADY ATE OR DRANK (past tense, actually consumed) — not a craving, not something she's considering, not a question, not a hypothetical. If she is not clearly describing food already consumed, return an empty foods array. If she mentions the same food more than once in the message, that almost always means she's restating herself, not that she ate it multiple times — list it ONCE with servings reflecting one normal portion, unless she explicitly signals more than one (\"another bowl\", \"a second plate\", \"twice\", \"ate it again later\"). When in doubt, assume one serving. Return each distinct food item with a realistic calorie/macro estimate for the amount described (assume a standard serving if vague). Return integers for calories and grams.",
      tools: [RECORD_FOODS_TOOL],
      tool_choice: { type: 'tool', name: 'record_foods' },
